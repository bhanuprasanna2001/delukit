...
log = logging.getLogger("delukit.bronze")


def run(config_path: str) -> PipelineConfig:
    """Fetch and land all sources from a pipeline config into bronze storages."""
    config = load_pipeline_config(config_path)
    end = resolve_day(config.end, config.timezone)
    begin = resolve_day(config.start, config.timezone)
    t0 = time.perf_counter()
    log.info(
        "┌ bronze start · window=%s..%s · sources=%s · storages=%s",
        begin,
        end,
    ...
    for name, source_config in config.sources.items():
        ...
        if first_day > end:
            ...
            continue
        log.debug("%s: fetching %s..%s", name, first_day, end)
        source = build_source(name, source_config, config.timezone)
        t1 = time.perf_counter()
        try:
            records = _fetch_source(
                source, name, source_config, first_day, end, fetched_at
            )
    ...
    if failures:
        log.error(
            "└ bronze failed · %d fetched · %s · %d synced · %d failed: %s",
            total_records,
            elapsed,
            synced,
            len(failures),
    ...
    log.info(
        "└ bronze complete · %d fetched · %s · %d synced",
        total_records,
        elapsed,
        synced,
    )
...
def sync(config_path: str) -> PipelineConfig:
